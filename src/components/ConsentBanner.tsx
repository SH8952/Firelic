...
/**
 * Google Consent Mode v2용 쿠키 동의 배너 (firelic 버전 — shadcn/ui가 없는
 * 저장소라 다른 컴포넌트들과 동일하게 순수 <button> + CSS 변수 클래스로
 * 작성). ExifLens/FlyDroneMap의 동일한 컴포넌트와 로직은 100% 동일하고
 * 스타일만 firelic 디자인 시스템에 맞춤.
 *
 * `needsConsent`는 서버 컴포넌트(레이아웃)에서 방문자의 국가(Vercel의
 * x-vercel-ip-country 헤더)를 보고 미리 계산해 내려준다 — EEA/영국/스위스가
 * 아니면 gtag의 consent 기본값이 애초에 "거부"로 설정되지 않으므로
 * (GoogleAnalytics.tsx의 `region` 옵션 참고) 이 배너도 띄우지 않는다.
 *
 * 선택 상태는 useSyncExternalStore로 로컬스토리지를 읽어 관리한다(setState를
 * 이펙트 안에서 직접 호출하지 않기 위함, 서버/클라이언트 하이드레이션 불일치
 * 방지 목적).
 */
export function ConsentBanner({ needsConsent }: { needsConsent: boolean }) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 border-t border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-lg">
      <div className="mx-auto flex max-w-5xl flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex shrink-0 gap-2">
          <button
            ...
            className="rounded-md border border-[var(--color-border)] px-4 py-2 text-sm font-medium text-[var(--color-text-primary)] transition-colors hover:text-[var(--color-primary)]"
          >
            {t("reject")}
          </button>
          <button
            ...
            className="rounded-md bg-[var(--color-primary)] px-4 py-2 text-sm font-medium text-white transition-opacity hover:opacity-90"
          >
            {t("accept")}
          </button>
        </div>
