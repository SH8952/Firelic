...
ExifLens(automation/attach-guide-image.py)와 동일한 설계를 FIRE Calculator(firelic)에
맞게 이식한 버전. firelic은 tags가 비어있을 수 있으므로(신규 가이드는 항상 tags를
채우는 것을 권장), tags가 없으면 slug 단어를 검색어로 대체한다.
...
def fetch_image(repo, slug, en_mdx_path):
    ...
    utm = "utm_source=FIRECalculator&utm_medium=referral"
