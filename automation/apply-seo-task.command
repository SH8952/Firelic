#!/bin/bash
# FIRE Calculator SEO 개선 자동 적용 스크립트 (영구 설치형)
...
#
# [2026-09-07 안전장치 추가 — FlyDroneMap에서 먼저 발견된 문제를 여기에도 동일하게
#  적용] 예전 버전은 git add -A(전체 스테이징)를 사용해, 이 스크립트 실행 시점에
# 저장소에 다른 미커밋 변경사항(예: 진행 중인 별도 기능 개발, 또는 payload zip
# 자체)이 남아있으면 그 변경사항까지 함께 커밋/push되거나, 겹치는 파일이 있으면
# 조용히 덮어써져 유실되는 문제가 실제로 발생할 수 있었습니다(FlyDroneMap에서
# 실제로 발생). 이를 방지하기 위해 두 가지 안전장치를 추가했습니다: (A) 실행 전
...
# --- 3. 작업 전 백업 ---
BACKUP_DIR="$REPO/_backups/firelic_backup_$(date +%Y%m%d_%H%M%S)"
...
#     파일만 정확히 골라서 add — 다른 무관한 변경사항(payload zip 자체 포함)이
#     섞여 들어갈 수 없음 ---
while IFS= read -r rel; do
  [ -z "$rel" ] && continue
  git add -- "$rel"
