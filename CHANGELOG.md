...
- 안전 확인: 사용자는 자산운용 전문가가 아니고 사이트는 투자 자문업이 아니므로, 콘텐츠는 특정 종목·코인·상품을 단정적으로 추천하지 않고 "예를 들어 ~라면 이렇게 나눠 생각해볼 수 있다"는 예시·비교 형식으로만 작성하기로 사용자와 합의(기존 "투자 자문이 아니다" 면책 문구와 일관성 유지)
- 수정: `src/lib/guides.ts` — `CATEGORY_ORDER`에 "Practical Wealth-Building Strategies"(실전 자산증식 전략) 카테고리 추가(FIRE Basics & Concepts 바로 다음 순서)
- 수정: `automation/guide-topics-queue.json` — 위 카테고리로 신규 주제 25개(order 25~49) 추가: 소득 구간별 전략(최저임금/월급 300·400만원), 금융상품 비교(주식·예적금·코인·보험·부동산 조각투자·ETF), 실전 의사결정(빚 상환 vs 투자, 부수입 vs 지출 절감, 신용점수·대출 활용, 자동이체 습관화 등). description 필드도 최신 카테고리 목록과 향후 작성 방향으로 갱신
- Claude Code Remote 예약 작업(`trig_019Qtn3nfb5UzbustuAT86et`) 프롬프트 갱신: (1) 카테고리 무관하게 앞으로 모든 신규 글은 개념 설명보다 구체적 소득·시나리오 기반 실전 예시를 우선하도록 지시, (2) 특정 상품/종목/코인을 단정적으로 추천하지 않고 예시·비교 형식만 쓰도록 명시, (3) 원화 기준 구체적 금액이 들어간 주제는 다른 언어판에서 기계적 직역 대신 그 나라 독자에게 맞는 소득 구간으로 각색하도록 안내, (4) 미발행 주제가 5개 이하로 남으면 최종 보고에 경고를 포함하도록 추가(향후 보충 필요 시점을 놓치지 않기 위함)
- 검증: `npx tsc --noEmit`(오류 0건), `npx eslint src`(오류 0건), `guide-topics-queue.json` JSON 유효성 확인, 신규 슬러그 25개 기존 슬러그와 중복 없음 확인
- 작업 전 `_backups/backup_20260920_230722/`로 백업 생성
- 커밋: `03979cc` ("feat: 실전 자산증식 전략 카테고리 신설 + 발행 대기열에 현실적인 주제 25개 추가")
- 운영 방식: "무제한 발행"은 큐 파일 특성상 한 번에 무한히 채울 수 없어, 향후 미발행 주제가 얼마 남지 않으면(예약 작업이 5개 이하 도달 시 경고) Claude가 사용자 요청 없이 먼저 같은 방향의 새 배치를 추가로 채워 넣는 방식으로 운영하기로 합의(사용자가 중단을 요청하기 전까지 계속)
...
- 수정: `src/components/SiteFooter.tsx` — 바깥쪽을 `<div className="... flex ... justify-between ... sm:flex-row">`로 감싸고, 정책 링크는 안쪽 `<nav>`로, `<VisitorCounter />`는 그 옆의 별도 flex 항목으로 분리(ExifLens/FlyDroneMap과 동일한 좌:링크 — 우:카운터 배치 구조).
- 검증: `npx tsc --noEmit`, `npx eslint`, `npm run build`(전체 빌드, SSG 유지 확인) 모두 통과.
...
- 배경: ExifLens/FlyDroneMap에 적용한 것과 동일한 기능을 firelic에도 동일하게 적용. 개발자 본인 방문(집 + 외부에서 모바일 북마크로 접속하는 경우 모두)을 방문자 집계/GA에서 제외하되, IP 기반이 아니라 네트워크·위치와 무관하게 동작하는 쿠키 기반 방식으로 구현.
- 구조 차이 반영: firelic은 다른 두 프로젝트와 달리 `src/proxy.ts`가 아니라 `src/middleware.ts`(geo-country 쿠키 로직 없이 next-intl 미들웨어만 감싸는 형태)를 사용하고, GA4도 `layout.tsx` 인라인 대신 `src/components/GoogleAnalytics.tsx` + `src/lib/analytics.ts`로 분리되어 있어, 이 두 파일 각각에 맞춰 구현.
- 추가: `src/middleware.ts` — `?dev=<DEV_EXCLUDE_TOKEN>` 쿼리 파라미터로 접속하면 1년 만료 `dev_exclude` 쿠키를 심는 로직 추가.
- 추가: `src/components/GoogleAnalytics.tsx` — GA4 인라인 스크립트에서 `dev_exclude` 쿠키가 있으면 `gtag('config', ...)` 호출(및 그로 인한 페이지뷰 수집)을 건너뛰도록 수정.
- 추가: `src/lib/visitor-counter.ts`, `src/app/api/visitor-count/route.ts` — Vercel KV(Upstash Redis)를 raw fetch(REST API)로 호출해 방문자 수를 증가/조회. `dev_exclude` 쿠키가 있으면 증가 없이 조회만 수행. Redis 키는 `firelic:visitor_count`로, ExifLens/FlyDroneMap과 공유하는 Vercel KV 인스턴스 1개를 프로젝트별 키 접두어로 구분해서 사용.
- 추가: `src/components/VisitorCounter.tsx` — 클라이언트 컴포넌트로 카운트를 fetch해 표시. `src/components/SiteFooter.tsx` 하단 링크 나열에 함께 배치.
